...
def ang_rect(sc, x0, y0, w, h, angle, color, w1=0):
    x1 = x0 + w * 0.5 * math.cos((math.pi / 180) * angle) - h * 0.5 * math.sin((math.pi / 180) * angle)
    y1 = y0 + h * 0.5 * math.cos((math.pi / 180) * angle) + w * 0.5 * math.sin((math.pi / 180) * angle)
    x2 = x0 - w * 0.5 * math.cos((math.pi / 180) * angle) - h * 0.5 * math.sin((math.pi / 180) * angle)
    y2 = y0 + h * 0.5 * math.cos((math.pi / 180) * angle) - w * 0.5 * math.sin((math.pi / 180) * angle)
    x3 = x0 - w * 0.5 * math.cos((math.pi / 180) * angle) + h * 0.5 * math.sin((math.pi / 180) * angle)
    y3 = y0 - h * 0.5 * math.cos((math.pi / 180) * angle) - w * 0.5 * math.sin((math.pi / 180) * angle)
    x4 = x0 + w * 0.5 * math.cos((math.pi / 180) * angle) + h * 0.5 * math.sin((math.pi / 180) * angle)
    y4 = y0 - h * 0.5 * math.cos((math.pi / 180) * angle) + w * 0.5 * math.sin((math.pi / 180) * angle)
...
def ang_rect2(sc, x0, y0, w, h, angle, quarter
    ...
    if quarter == 3:
        bx = -1
    x0 = x0 - (bx * w * 0.5 * math.cos((math.pi / 180) * angle) - by * h * 0.5 * math.sin((math.pi / 180) * angle))
    y0 = y0 - (by * h * 0.5 * math.cos((math.pi / 180) * angle) + bx * w * 0.5 * math.sin((math.pi / 180) * angle))
...
def hair(r_head, r_tr, phi, n, x0, y0, sc, color_fill, color_side):
    for i in range(n):
        ...
        x1 = int(x0 + r_head * math.cos((math.pi / 180) * alpha) + math.sqrt(3) * 0.5 * r_tr * math.sin((math.pi / 180) * alpha))
        y1 = int(y0 + r_head * math.sin((math.pi / 180) * alpha) - math.sqrt(3) * 0.5 * r_tr * math.cos((math.pi / 180) * alpha))
        x2 = int(x0 + r_head * math.cos((math.pi / 180) * alpha) - math.sqrt(3) * 0.5 * r_tr * math.sin((math.pi / 180) * alpha))
        y2 = int(y0 + r_head * math.sin((math.pi / 180) * alpha) + math.sqrt(3) * 0.5 * r_tr * math.cos((math.pi / 180) * alpha))
        x3 = int(x0 + (r_head + r_tr * 1.5) * math.cos((math.pi / 180) * alpha))
        y3 = int(y0 + (r_head + r_tr * 1.5) * math.sin((math.pi / 180) * alpha))
...
def nose(x0, y0, r_nose, color_nose, sc):
    x1 = int(x0)
    y1 = int(y0 + r_nose)
    x2 = int(x0 + math.cos(math.pi / 6) * r_nose)
    y2 = int(y0 - math.sin(math.pi / 6) * r_nose)
    x3 = int(x0 - math.cos(math.pi / 6) * r_nose)
    y3 = int(y0 - math.sin(math.pi / 6) * r_nose)
...
def boys(color_hair, color_eyes, color_t_shirt, color_skin, color_border, color_mouth, color_pup, color_nose,
                  size, sc, x0, y0):
    angle_sh = 70
    angle_pl_sh = 40
    ...
    x1 = int(0.5 * size - 0.3 * size * math.cos((math.pi / 180) * angle_pl_sh) + x0)
    y1 = int(size - 0.3 * size * math.sin((math.pi / 180) * angle_pl_sh) + y0)
    x2 = int(0.5 * size + 0.3 * size * math.cos((math.pi / 180) * angle_pl_sh) + x0)
    y2 = int(size - 0.3 * size * math.sin((math.pi / 180) * angle_pl_sh) + y0)
    A1 = []
    for i in range(5):
        A1.append((int(x1 + 0.1 * size * math.cos((math.pi / 180) * (angle_sh + 73 * i))),
                   int(y1 - 0.1 * size * math.sin((math.pi / 180) * (angle_sh + 73 * i)))))
    ...
    A2 = []
    for i in range(5):
        A2.append((int(x2 - 0.1 * size * math.cos((math.pi / 180) * (angle_sh + 73 * i))),
                   int(y2 - 0.1 * size * math.sin((math.pi / 180) * (angle_sh + 73 * i)))))
...
LOR = (255, 215, 0)
screen.fill(WHT)
boys(LOR, GGR, DGN, SKN, BLC, RED, BLC, BRW, SIZE, screen, int(SIZE * 0.015), 0)
boys(PNK, GBL, ORG, SKN, BLC, RED, BLC, BRW, SIZE, screen, int(SIZE * 0.885), 0)
